...
import com.rentservice.rentserVice.Adaptar.Category_Spinner_Adaptar;
...
import com.rentservice.rentserVice.Data_Model.EditPost.EditPostContainer;
...
public class Edit_Post extends AppCompatActivity {
    ...
    String ReceiveId, ReceiveTitle, ReceiveDescription, ReceivePrice, ReceiveAddress, ReceiveDivision,
            ReceiveDistrict, ReceivePoliceStation, ReceiveCategory;
    ...
    public void SetData() {
        ...
        ReceiveId = getIntent().getStringExtra("Id");
        ReceiveTitle = getIntent().getStringExtra("Title");
        ReceiveDescription = getIntent().getStringExtra("Description");
        ReceivePrice = getIntent().getStringExtra("Price");
        ReceiveAddress = getIntent().getStringExtra("Address");
        ReceiveDivision = getIntent().getStringExtra("Division");
        ReceiveDistrict = getIntent().getStringExtra("District");
        ReceivePoliceStation = getIntent().getStringExtra("PoliceStation");
        ReceiveCategory = getIntent().getStringExtra("Category");
//        Imageone = getIntent().getStringExtra("ImageOne");
//        ImageTwo=("https://rentservicebd.com/public/api/image/"+Imageone);
//        Glide.with(getApplicationContext()).load(ImageTwo).into(ImageOne);


        Title.setText(ReceiveTitle);
        Description.setText(ReceiveDescription);
        Price.setText(ReceivePrice);
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == Constant.PICK_PHOTO_ONE && data != null) {
            ...
            try {
                ...
                //ImageTwo= Base64.encodeToString(bitmapdata,Base64.NO_WRAP);
...
//    }
    ...
    public void register() {
        ...
        api.EditPost(requestBody).enqueue(new Callback<EditPostContainer>() {
            @Override
            public void onResponse(Call<EditPostContainer> call, Response<EditPostContainer> response) {
                if (response.isSuccessful() && response.body() != null) {
                    progressDialog.dismiss();
            ...
            @Override
            public void onFailure(Call<EditPostContainer> call, Throwable t) {
                progressDialog.dismiss();
                // finish();
                Toast.makeText(getApplicationContext(), t.toString(), Toast.LENGTH_SHORT).show();
            }
